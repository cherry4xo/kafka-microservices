...
async def on_event(stream) -> None:
    async for msg_key, msg_value in stream.items():
        ...
        logger.info(f"Received new processes data message {msg_value}")
        servialized_message = json.loads(msg_value)
        for pair_name, pair_value in servialized_message.items():
            ...
            logger.info(f"Aggregated value: {pair_value}")
